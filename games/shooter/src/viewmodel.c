...
static void DrawCombinedRigViewmodel(Camera camera, int wi) {
    ...
    up = Vector3Normalize(Vector3CrossProduct(right, fwd));  // re-orthonormalize

    // Optional base roll (cant) rotates (right, up) about fwd.
    if (CRIG_BASE_ROLL != 0.0f) {
        float cr = cosf(CRIG_BASE_ROLL), sr = sinf(CRIG_BASE_ROLL);
        Vector3 r2 = {
            right.x * cr + up.x * sr,
            right.y * cr + up.y * sr,
            right.z * cr + up.z * sr,
        ...
        right = Vector3Normalize(r2);
        up    = Vector3Normalize(u2);
    }

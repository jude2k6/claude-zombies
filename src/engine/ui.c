// ui.c — implementation of the house UI toolkit. raygui only; never a game
...
void Eng_UiApplyTheme(void) {
    ...
    GuiSetStyle(DEFAULT, BORDER_COLOR_FOCUSED, ColorToInt(ENG_UI_GOLD));
    GuiSetStyle(DEFAULT, BORDER_COLOR_PRESSED, ColorToInt(ENG_UI_GOLD));
    GuiSetStyle(DEFAULT, TEXT_COLOR_NORMAL,    ColorToInt((Color){ 210, 214, 224, 255 }));
    GuiSetStyle(DEFAULT, TEXT_COLOR_FOCUSED,   ColorToInt(ENG_UI_GOLD));
    GuiSetStyle(DEFAULT, TEXT_COLOR_PRESSED,   ColorToInt(WHITE));
    GuiSetStyle(BUTTON, BORDER_WIDTH, 2);
}

void  Eng_UiSetScale(float s) { g_scale = (s < 0.1f) ? 0.1f : s; }
float Eng_UiScale(void)       { return g_scale; }

float Eng_UiRecommendedScale(void) {
...
void Eng_UiText(const char *t, float x, float y, int basePx, Color c) {
    DrawText(t, (int)x, (int)y, (int)(basePx * g_scale), c);
}

void Eng_UiTextShadow(const char *t, float x, float y, int basePx, Color c) {
    int fs = (int)(basePx * g_scale);
    DrawText(t, (int)x + 1, (int)y + 2, fs, (Color){ 0, 0, 0, 170 });
    DrawText(t, (int)x, (int)y, fs, c);
}

void Eng_UiTextCentered(const char *t, float cx, float y, int basePx, Color c) {
    int fs = (int)(basePx * g_scale);
    Eng_UiTextShadow(t, cx - MeasureText(t, fs) / 2.0f, y, basePx, c);
}
...
bool Eng_UiToolButton(Rectangle r, const char *label, bool active) {
    if (active)
        DrawRectangleRec((Rectangle){ r.x - 5 * g_scale, r.y, 3 * g_scale, r.height }, ENG_UI_GOLD);
